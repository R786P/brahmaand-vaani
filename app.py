...
# ✅ Static files serve karne ke liye explicit route
@app.route('/assets/gods/<filename>')
def serve_god_images(filename):
    return send_from_directory('assets/gods', filename)
...
def chat():
    try:
        ...
        if not client:
            ...
            return jsonify({"reply": "⚠️ API not configured. Check HF_API_KEY"}), 500
...
def elevenlabs_tts():
    try:
        ...
        if not ELEVENLABS_API_KEY:
            logger.error("❌ ElevenLabs key missing")
