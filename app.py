...
# Initialize Groq Client (Chat ke liye)
groq_client = None
if GROQ_API_KEY:
    try:
...
# Initialize Hugging Face Client (Video/Image ke liye)
hf_client = None
if HF_API_KEY:
    try:
...
def elevenlabs_tts():
    """Sirf Audio generate (CSS Animation ke saath)"""
    try:
        data = request.json
        text = data.get('text', '')
        voice_id = data.get('voice_id', '21m00Tcm4TlvDq8ikWAM')
